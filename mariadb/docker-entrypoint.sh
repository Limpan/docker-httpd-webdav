...
docker_process_sql() {
	...
	mysql --defaults-extra-file=<( _mysql_passfile "${passfileArgs[@]}") --socket="${SOCKET}" -uroot -hlocalhost --socket="${SOCKET}" "$@"
}
...
#    docker_process_sql --database=mysql <<<"INSTALL SONAME 'auth_bcrypt.so';"
	fi
}
...
_main() {
  ...
  mysql_note "Generating populate.sql."
  /usr/local/bin/user sql > /tmp/populate.sql

  mysql_note "Starting temporary server."
  docker_temp_server_start "$@"
  mysql_note "Temporary server started."

  mysql_note "Creating users and databases."
  docker_process_sql --database=mysql <<<`cat /tmp/populate.sql`
